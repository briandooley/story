...
def get_jira_issues(jira_instance, jql_query, api_token):
    ...
    params = {"jql": jql_query.strip(), "fields": "customfield_12310243,key", "maxResults": 1000}
...
def calculate_story_points(issues):
    ...
    for issue in issues.get("issues", []):
        story_points = issue["fields"].get("customfield_12310243")  # customfield_12310243 is the "Story Points" field
    
        if isinstance(story_points, (int, float)):
            total_points += story_points
        else:
            issues_without_points.append(issue["key"])
...
def main():
    ...
    try:
        ...
        total_story_points, issues_without_points = calculate_story_points(issues)
        
        print(f"Total Story Points: {total_story_points}")
        if issues_without_points:
            print("Issues without story points:")
            for issue in issues_without_points:
                print(f"- {issue}")
